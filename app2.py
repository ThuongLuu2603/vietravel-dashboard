...
# --- CẤU HÌNH TRANG (WIDE MODE & COLLAPSED SIDEBAR) ---
st.set_page_config(layout="wide", page_title="Vietravel One-Page View", initial_sidebar_state="collapsed")

# --- CSS TỐI ƯU KHÔNG GIAN (ZERO MARGIN) ---
st.markdown("""
<style>
    /* Đẩy sát lề trên, giảm padding */
    .block-container {padding-top: 1rem; padding-bottom: 0rem; padding-left: 1rem; padding-right: 1rem;}
    
    /* Style cho Card chỉ số nhỏ gọn */
    .metric-card {
...
with c_title:
    st.markdown("### 🚁 VIETRAVEL STRATEGIC DASHBOARD (ONE-PAGE VIEW)")
with c_filter:
    period = st.selectbox("", ["Tháng 11/2025", "Quý 4/2025", "Năm 2025"], label_visibility="collapsed")

# --- HÀNG 1: 5 CHỈ SỐ KEY METRICS (Ngang hàng trên cùng) ---
# Tạo hàm vẽ thẻ metric nhanh
...
# --- HÀNG 2: CHIA LƯỚI 4 CỘT (CỐT LÕI CỦA ONE-PAGE) ---
# Col 1: Sales (Quan trọng nhất - Rộng hơn xíu)
# Col 2: Finance
# Col 3: Market
# Col 4: HR/Ops
...
with col_biz:
    ...
    # 1. KPI Achievement (Cao nhất để dễ nhìn)
    entities = ['Cty', 'HO', 'Bắc', 'Trung', 'Tây'] # Tên viết tắt cho gọn
    fig_kpi = go.Figure()
    # Helper add trace
    ...
    # 3. Growth vs Industry (Compact)
    fig_gr = go.Figure()
    fig_gr.add_trace(go.Bar(name='Vietravel', x=['Q1','Q2','Q3'], y=[15,20,25], marker_color='#0051a3'))
...
with col_fin:
    ...
    # 2. EBITDA Combo
    fig_eb = go.Figure()
    fig_eb.add_trace(go.Bar(x=['T9','T10','T11'], y=[40,45,50], marker_color='#2ca02c', name="EBITDA"))
...
with col_mkt:
    ...
    # 1. RMS Bubble (Nổi bật)
    df_rms = pd.DataFrame({"Tuyến": ["Đ.Bắc Á", "Âu Úc", "ĐNA", "Nội địa"], "RMS": [0.8, 1.2, 1.5, 0.9], "Gr": [15,10,5,8], "Pax": [15, 8, 25, 40]})
    ...
    fig_str = px.bar(df_struc, x="Năm", y="Rev", color="Mảng", color_discrete_map=COLOR_MAP)
...
with col_hr:
    ...
    # 1. ROI Marketing (Đưa lên đầu như yêu cầu)
